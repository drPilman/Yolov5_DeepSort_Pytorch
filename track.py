...
from yolov5.utils.general import (LOGGER, check_img_size, non_max_suppression,
                                  ...
                                  increment_path, strip_optimizer, colorstr)
...
def detect(opt):
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                t4 = time_sync()
                outputs[i] = deepsort_list[i].update(xywhs.cpu(), confs.cpu(),
                                                     clss.cpu(), im0)
                t5 = time_sync()
                dt[3] += t5 - t4
                        ...
                        if save_txt:
                            ...
                            with open(txt_path + '.txt', 'a') as f:
                                f.write(('%g ' * 10 + '\n') % (
                                    ...
                                    -1,
                                    i))
                        if redis_id or save_vid or save_crop or show_vid:  # Add bbox to image
    ...
    if update:
        strip_optimizer(
            yolo_model)  # update model (to fix SourceChangeWarning)
